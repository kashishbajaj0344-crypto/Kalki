...
# Backwards-compatibility aliases for older code that referenced different status names
AgentStatus.INITIALIZED = AgentStatus.INITIALIZING
...
class BaseAgent(ABC):
    ...
    async def subscribe_to_event(self, event_type: str, callback):
        ...
        if self.event_bus:
            # EventBus.subscribe is a synchronous registration in this project.
            # Support both sync and async subscribe implementations for compatibility.
            res = self.event_bus.subscribe(event_type, callback)
