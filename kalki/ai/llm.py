...
from kalki.core.utils.config import get_config
from kalki.core.utils.logging_config import get_logger
import torch
...
import numpy as np
from kalki.cognition.meta_core import get_meta_core, process_command
from kalki.learning.learning.vectordb import BGEEmbedder
...
try:
    ...
    logger.warning("⚠️  bitsandbytes not installed - quantization disabled. Install: pip install bitsandbytes")
...
class LlamaEngine:
    ...
    def __init__(
        ...
        max_context_length: int = 128000  # Full 128K context
    ):
        ...
        self.quantization = quantization if BITSANDBYTES_AVAILABLE else "none"
        ...
        self.batch_queue = []  # For continuous batching
        self.batch_size = 5
    ...
    async def _try_load_local_model(self, model_path: str) -> bool:
        """Try to load a local fine-tuned model with FULL OPTIMIZATIONS"""
        try:
            logger.info(f"🚀 Loading local model with optimizations from: {model_path}")
            logger.info(f"   Quantization: {self.quantization}")
            logger.info(f"   Compilation: {self.compile_model}")
            ...
            if hasattr(self.tokenizer, 'model_max_length'):
                self.tokenizer.model_max_length = self.max_context_length

            # Setup quantization if requested
            quantization_config = None
            if self.quantization in ["int8", "int4"] and BITSANDBYTES_AVAILABLE:
                try:
                    if self.quantization == "int8":
                        ...
                        logger.info("✅ Using int8 quantization (~8 GB memory)")
                        ...
                        logger.info("✅ Using int4 quantization (~4 GB memory)")
                except Exception as e:
            ...
            # Load model from local path with optimizations
            load_kwargs = {
            ...
            if self.compile_model:
                try:
                    ...
                    logger.info("✅ Model compiled successfully (20-30% speedup expected)")
                except Exception as e:
            ...
            logger.info(f"✅ Model loaded. Estimated memory: {memory_estimate:.1f} GB")
            logger.info("🚀 FULL OPTIMIZATIONS ACTIVE!")
    ...
    async def generate(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            # Reuse KV cache if available (for faster multi-turn conversations)
            ...
            kwargs.pop("use_cache", None)  # Remove from kwargs to avoid conflicts
            ...
            try:
                ...
                # Store KV cache for next turn (if available)
                if use_cache and hasattr(outputs[0], 'past_key_values'):
                    self.kv_cache = outputs[0].past_key_values
                elif use_cache and isinstance(outputs, list) and len(outputs) > 0:
                    # Try to extract cache from pipeline output
...
class LlamaVisionEngine:
    ...
    Now with FULL OPTIMIZATIONS:
    - Quantization (int4 recommended for vision) for 4x memory reduction
    - Model compilation for 20-30% speedup
    ...
    def __init__(
        self, 
        model_path: Optional[str] = None,
        quantization: str = "int4",  # int4 recommended for vision (6GB vs 22GB)
        compile_model: bool = True,
        use_flash_attention: bool = True
    ):
        # ALWAYS prioritize local models from kalki/models directory
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"🚀 Loading local Llama 3.2 Vision 11B with optimizations from: {self.model_path}")
            logger.info(f"   Quantization: {self.quantization}")
            logger.info(f"   Compilation: {self.compile_model}")
            logger.info(f"   Flash Attention: {self.use_flash_attention}")
            ...
            # Setup quantization if requested (int4 recommended for vision)
            quantization_config = None
            if self.quantization in ["int8", "int4"] and BITSANDBYTES_AVAILABLE:
                try:
                    if self.quantization == "int4":
                        ...
                        logger.info("✅ Using int4 quantization for vision (~6 GB memory)")
                    elif self.quantization == "int8":
                        quantization_config = BitsAndBytesConfig(
                            load_in_8bit=True,
                            llm_int8_threshold=6.0
                        )
                        logger.info("✅ Using int8 quantization for vision (~11 GB memory)")
                except Exception as e:
                    logger.warning(f"⚠️  Vision quantization setup failed: {e}")
                    quantization_config = None
            ...
            if quantization_config:
                ...
                torch_dtype = torch.bfloat16 if self.device == "mps" else (torch.float16 if self.device == "cuda" else torch.float32)
            ...
            if self.use_flash_attention:
                load_kwargs["use_flash_attention_2"] = True
                load_kwargs["attn_implementation"] = "flash_attention_2"
                logger.info("✅ Flash Attention 2 enabled for vision")
            ...
            if self.compile_model:
                try:
                    logger.info("🔧 Compiling vision model for faster inference...")
                    self.model = torch.compile(
                        self.model,
                        mode="reduce-overhead",
                        fullgraph=False
                    )
                    logger.info("✅ Vision model compiled successfully")
                except Exception as e:
                    logger.warning(f"⚠️  Vision model compilation failed: {e}")
            
            # Estimate memory usage
            memory_estimate = self._estimate_memory_usage()
            logger.info(f"✅ Vision model loaded. Estimated memory: {memory_estimate:.1f} GB")
            logger.info("🚀 FULL OPTIMIZATIONS ACTIVE for vision!")
    ...
    def _estimate_memory_usage(self) -> float:
        """Estimate vision model memory usage in GB"""
        if self.quantization == "int4":
            return 6.0  # ~6 GB for 11B vision model with int4
        elif self.quantization == "int8":
            return 11.0  # ~11 GB for 11B vision model with int8
        else:
            return 22.0  # ~22 GB for 11B vision model with fp16
        ...
        try:
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    **inputs,
                    max_new_tokens=1024,  # Increased for better descriptions
                    temperature=0.7,
                    top_p=0.9,
                    top_k=40,
                    do_sample=True,
...
class LLMEngine:
    ...
    async def generate(
        ...
        **kwargs
        ...
        if use_advanced_reasoning and not image_path:
            try:
                from kalki.safety.advanced_reasoning import AdvancedReasoningEngine, ReasoningMethod
    ...
    async def _load_domain_model(self):
        ...
        try:
            from kalki.safety.domain_finetuning import DomainFineTuner
    ...
    async def generate_batch(self, prompts: List[str], **kwargs) -> List[str]:
        """Batch generate for multiple prompts with continuous batching optimization"""
        if not self.pipe:
            return ["Error: Model not initialized"] * len(prompts)
        
        # For now, process sequentially but with optimized settings
        # TODO: Implement true continuous batching when transformers supports it
        results = []
        for i, prompt in enumerate(prompts):
            # Disable KV cache reuse between different prompts in batch
            prompt_kwargs = kwargs.copy()
            if i > 0:  # Clear cache for new prompts
                prompt_kwargs["use_cache"] = False
                self.kv_cache = None
            
            result = await self.generate(prompt, **prompt_kwargs)
            results.append(result)
        
        return results
    
    def clear_cache(self):
        """Clear KV cache and memory cache"""
        self.kv_cache = None
        if self.device == "mps":
            torch.mps.empty_cache()
        elif self.device == "cuda":
